...
public class IntStack {
	...
	//생성자(메서드)
	public IntStack(int maxlen) { //스택을 생성할 떄 크기를 지정할 수 있도록!
	...
	public int push(int x) { //원하는 값(x)를 스택에 삽입, 0번 인덱스부터 차근차근 채우기!
		...
		return x;		//ptr++ : 데이터 개수 카운팅, 그다음 데이터는 그다음칸에 들어갈수 있도록!
	}
	
	
	//데이터 꺼내기 : 위(뒤)쪽에서만 꺼내는 작업 가능
	public int pop() {
		return stk[--ptr]; //포인터가 가리키는 값 -1먼저 시킨 후 데이터 반환
	}
	...
	public int indexOf(int x) { //x : 찾고 싶은 값
		...
		return -1; //찾고자하는 값이 스택에 없는경우에는 -1반환
	}
