...
public class WritingThread extends Thread{//멀티쓰레딩 -> 사용자들이 동시에 채팅 가능
   ...
   // 필드 : 소켓 -> import
   private Socket socket; //클라이언트가 채팅을 보낼 서버의 소켓 정보 저장 
                          //목적지
   ...
   // run()메소드 : 쓰레드가 수행할 한가지 작업 
   public void run() { //클라이언트가 작성한 채팅을 서버로 보내기
      //클라이언트가 작성한 채팅(데이터)을 서버로 전송 할 수 있는 통로(스트림)
      // 클라이언트 -> 서버로 나가는 것이기 때문에 Output(클라이언트 입장에서 생각)
      try {
         ...
         // sockert정보가 잘못됐을 경우에는 스트림이 정상적으로 생성되지 않을 수 있으므로 
         // 예외처리 해줘야함!
         e.printStackTrace();
      }
   }
